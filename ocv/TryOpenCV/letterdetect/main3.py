...
from drawtext import TrainingText, startindex, numofletters
...
size = (10, 20)#size of image for NN
...
image = TrainingText()#cv.LoadImage(IMAGE_NAME)
...
ds = ClassificationDataSet( size[0] * size[1] , 1, nb_classes=numofletters)#SupervisedDataSet(150, 10)


#10 samples for learning
print 'Start learning'
for i in xrange(0, numofletters * 5):    
    ds.addSample(GetInp(images[i], size), [i % numofletters])
    print '#', chr(i % 10)

ds._convertToOneOfMany()

net = buildNetwork( ds.indim, 800, ds.outdim, outclass=SoftmaxLayer )#buildNetwork(150, 400, 200, 10, bias=True)
...
for i in xrange(numofletters * 5, numofletters * 8):
    cv.ShowImage('img', images[i])
    outdata = ClassificationDataSet(size[0] * size[1],1, nb_classes=numofletters)
    outdata.addSample(GetInp(images[i], size), [0])
    outdata._convertToOneOfMany()
    out = net.activateOnDataset(outdata)
    out = out.argmax(axis=1)
    
    print 'out:', chr(startindex + out[0])
